...
The first word of a filename says what the script does. Scripts group into seven
stages, and the stages must run in order because each settles something the next
assumes.

| stage | scripts | purpose |
|---|---|---|
| **setup** | `check_environment.py`, `check_data.py`, `fit_physics_constants.py` | record the environment, describe the data, fit the physics constants |
| **decide** | `check_sanity.py`, `decide_split.py`, `decide_architecture.py`, `decide_optimizer.py`, `decide_budget.py` | settle every training setting, before any large run |
| **measure** | `confirm_settings.py`, `run_label_sweep.py`, `run_matched_prior.py`, `run_classical_baselines.py` | the main experiment: accuracy against how many labels were used |
| **ablate** | `ablate_loss_terms.py`, `ablate_design_choices.py`, `ablate_duplicates.py` | what each part of the model contributes |
| **validate** | `validate_physics.py`, `validate_generalization.py`, `validate_robustness.py`, `validate_explainability.py`, `validate_performance.py` | what the trained model can and cannot do |
| **tune** | `search_hyperparameters.py` | a large hyperparameter search, run after the main experiment |
| **report** | `make_tables.py`, `make_figures.py` | tables and figures |
...
`check_environment.py` records the interpreter, the package versions, the GPUs, a
